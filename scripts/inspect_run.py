...
def inspect_thread(thread_id: str):
    ...
    db_path = ROOT_DIR / DB_FILENAME
    if not db_path.exists():
        print(f"Error: SQLite database '{DB_FILENAME}' not found.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Inspect persistent SQLite checkpoint history for Sovereign Workbench thread.")
    ...
    if args.thread_id:
        ...
        if not recent:
            print(f"No active thread checkpoints found in '{DB_FILENAME}'. Run 'python server.py' or 'python cli.py' to generate runs.")
        else:
            print(f"\nRecent {len(recent)} Thread IDs in '{DB_FILENAME}':")
            for tid in recent:
                print(f"  • {tid}")
